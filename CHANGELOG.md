...
- **Schema row detail viewer**: a per-row **View** action (eye icon) on the preview tab opens a read-only dialog showing every field expanded — full values, pretty-printed JSON, per-field copy, and "Copy as JSON". New Vue component `RowDetailDialog.vue`. The per-row actions cell now renders for every engine, and View/Edit/Delete stay visible at all times (disabled with the reason on hover when an engine genuinely can't target a row).
- **Edit/Delete on keyless SQL tables**: SQL tables without a primary key are now editable and deletable. With no PK, rows are targeted by matching every column's original value (`NULL` → `IS NULL`); the full statement is shown in the confirm dialog for review first, and the toolbar warns that identical rows are affected together. The other engines still require their addressable id (`_id` / `_key` / Redis key).
